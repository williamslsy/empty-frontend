...
export interface Input {
  ...
  alert?: string;
  fullWidth?: boolean;
  classNames?: { parentClassName?: string; inputClassName?: string; labelClassName?: string; wrapperClassName?: string };
  isSearch?: boolean;
}

const Input = React.forwardRef<HTMLInputElement, InputProps & Input>(
  ({ className, classNames, label, error, startContent, endContent, isRequired, alert, fullWidth, isSearch, ...props }, ref) => {
    return (
      <>
        <div className={clsx("flex flex-col relative gap-1 w-fit", { "w-full": fullWidth }, classNames?.parentClassName)}>
          ...
          {error && (
            <p className={clsx(" text-xs w-full text-left flex-1 flex items-center gap-1 text-tw-error")}>
