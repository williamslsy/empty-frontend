...
export function createCoingeckoService(params: CreateCoingeckoParameters): CoingeckoServiceReturnType {
  ...
  async function getPrice(coinId: string): Promise<number> {
    try {
      ...
      const { usd } = await api.get(`/simple/price?ids=${coinId}&vs_currencies=usd`).json<{ usd: number }>();
