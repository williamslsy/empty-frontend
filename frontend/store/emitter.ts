...
export type EventData<eventMap extends EventMap, eventName extends keyof eventMap> = (eventMap[eventName] extends [never]
  ? unknown
  : eventMap[eventName]) & {
  uid: string;
};
...
export class Emitter<eventMap extends EventMap> {
  ...
  emit<key extends EventKey<eventMap>>(eventName: key, ...params: eventMap[key] extends [never] ? [] : [data: eventMap[key]]) {
    const data = params[0];
