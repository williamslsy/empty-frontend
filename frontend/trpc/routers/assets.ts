...
import { BaseCurrency, NativeCurrency, WithPrice } from "~/types/currency";
import { z } from "zod";

export const assetsRouter = createTRPCRouter({
  getAsset: createTRPCPublicProcedure.input(z.object({ denom: z.string() })).query<WithPrice<BaseCurrency>>(async ({ input, ctx }) => {
    const { denom } = input;
    ...
    const metadata = await publicClient.request.bank.denomMetadata(denom).catch(() => ({
      symbol: "UNKNOWN",
