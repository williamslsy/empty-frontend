import type { BaseCurrency, CW20Currency, PoolIncentive, PoolMetricSerialized } from '@towerfi/types';
import { isTestnet } from './global';

export const navLinks: { label: string; to: string; isDisabled?: boolean; isExternal?: boolean }[] = [{ label: 'Pools', to: '/pools' }, ...(isTestnet ? [{ label: 'Faucet', to: '/faucet' }] : [])];
